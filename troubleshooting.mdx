...
- Check the polling interval (`ipmi.poll_interval`, default 30s / `IPMILINK_IPMI_POLL_INTERVAL`);
...
This is intended fail-safe behavior, not a bug. See [Security](/security).

## The dashboard will not load

- Confirm the container or process is running and listening on the configured port
  (`server.port` / `IPMILINK_SERVER_PORT`, default 3000).
