...
- **Docker:** `/data` inside the container (persist it with a volume).
- **pip:** `~/.ipmideck/`.

This directory holds `config.yaml`, the SQLite database, and the encryption key file used
to protect your stored BMC credentials. See [Configuration](/configuration) for details.

## Next steps

- [Configuration](/configuration) — tune ports, polling intervals, and retention.
- [Supported Hardware](/supported-hardware) — confirm your BMC is covered.
