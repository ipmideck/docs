...
<Note>
  ...
  `/data` in plaats van CLI-vlaggen, zie [Configuratie](/nl/configuration).
</Note>
...
<Warning>
  Op Linux is `/data` niet schrijfbaar voor een normale gebruiker, dus een pip-installatie
  die als niet-root-gebruiker wordt gestart, mislukt bij het opstarten met
  `PermissionError: [Errno 13] Permission denied: '/data'`. Stel `IPMIDECK_DATA_DIR` in op
  een map die je zelf bezit (zie hieronder), of draai ipmideck
  [als service](/nl/installation#als-service-draaien-linux).
</Warning>
