...
<Note>
  ...
  `/data` em vez de flags da CLI, veja [Configuração](/pt/configuration).
</Note>
...
<Warning>
  No Linux, `/data` não é gravável por um utilizador normal, por isso uma instalação pip iniciada
  como utilizador não-root falha no arranque com
  `PermissionError: [Errno 13] Permission denied: '/data'`. Defina `IPMIDECK_DATA_DIR` para um
  diretório que lhe pertença (veja abaixo), ou execute o ipmideck
  [como serviço](/pt/installation#executar-como-serviço-linux).
</Warning>
