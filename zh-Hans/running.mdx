...
<Note>
  ...
  而不是 CLI 标志，请参阅 [配置](/zh-Hans/configuration)。
</Note>
...
<Warning>
  在 Linux 上，`/data` 对普通用户不可写，因此以非 root 用户启动的 pip 安装会在启动时以
  `PermissionError: [Errno 13] Permission denied: '/data'` 失败。请将 `IPMIDECK_DATA_DIR`
  设置为一个你拥有的目录（见下文），或将 ipmideck
  [作为服务](/zh-Hans/installation#作为服务运行-linux)运行。
</Warning>
