...
<Note>
  ...
  `/data` rather than CLI flags, see [Configuration](/en/configuration).
</Note>
...
<Warning>
  On Linux, `/data` is not writable by a normal user, so a pip install started as a non-root
  user fails at startup with `PermissionError: [Errno 13] Permission denied: '/data'`. Set
  `IPMIDECK_DATA_DIR` to a directory you own (see below), or run ipmideck
  [as a service](/en/installation#running-as-a-service-linux).
</Warning>
