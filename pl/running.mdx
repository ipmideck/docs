...
<Note>
  ...
  `/data` zamiast flag CLI, zobacz [Konfiguracja](/pl/configuration).
</Note>
...
<Warning>
  Na Linuksie `/data` nie jest zapisywalny dla zwykłego użytkownika, więc instalacja pip uruchomiona jako
  użytkownik nie-root kończy się przy starcie błędem `PermissionError: [Errno 13] Permission denied: '/data'`. Ustaw
  `IPMIDECK_DATA_DIR` na katalog, którego jesteś właścicielem (patrz poniżej), lub uruchom ipmideck
  [jako usługę](/pl/installation#uruchamianie-jako-usługa-linux).
</Warning>
