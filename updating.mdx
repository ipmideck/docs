...
and encryption key live in the [data directory](/configuration#where-config-lives), which
...
<Note>
  Take a [backup](/backup-restore) before a major upgrade. Restoring is then a matter of
  putting the data directory back if anything goes wrong.
</Note>

## Next steps

- [Backup & restore](/backup-restore): snapshot your data before upgrading.
- [Running ipmideck](/running): start, stop, and where the data directory lives per platform.
